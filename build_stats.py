...
def rpc_batch(calls, rpc_url, tries=6):
    """POST a JSON-RPC batch; returns list of result objects (or [] on failure).

    Public RPC endpoints rate-limit aggressively, so 429s get a long,
    Retry-After-aware backoff rather than an immediate retry.
    """
    ...
    for attempt in range(tries):
        req = urllib.request.Request(rpc_url, data=body, headers={
            ...
            "accept": "application/json",
            "User-Agent": os.environ.get("USER_AGENT", "dreamdex-stats/1.0"),
        })
        try:
            ...
            if e.code == 429 and attempt < tries - 1:
                wait = 0.0
                try: wait = float(e.headers.get("Retry-After", 0) or 0)
                except Exception: wait = 0.0
                time.sleep(wait or min(2 ** attempt + 1, 20))   # 2,3,5,9,17s
                continue
            if attempt == tries - 1:
                print(f"warn: rpc batch failed: {e}", file=sys.stderr); return []
...
def fetch_owners():
    """Owner per token via batched on-chain ownerOf() calls.

    OpenSea's *list* endpoints don't return ownership (only the single-NFT
    endpoint does), so pulling 9,910 owners from OpenSea would be 9,910
    requests. Batched JSON-RPC does the whole collection in ~25 requests.
    Tokens that don't exist revert; those calls come back as errors and are
    skipped, so the id range can safely overshoot.
    """
    contract = os.environ.get("CONTRACT", "0x4e1f41613c9084fdb9e34e11fae9412427480e56").strip()
    rpc_url  = os.environ.get("RPC_URL", "https://ethereum-rpc.publicnode.com").strip()
    tok_min  = int(os.environ.get("TOKEN_MIN", "1"))
    tok_max  = int(os.environ.get("TOKEN_MAX", "9910"))
    batch    = int(os.environ.get("RPC_BATCH", "250"))
    workers  = int(os.environ.get("RPC_WORKERS", "2"))
    SELECTOR = "0x6352211e"          # ownerOf(uint256)

    ids = list(range(tok_min, tok_max + 1))
    chunks = [ids[i:i + batch] for i in range(0, len(ids), batch)]

    def run(chunk):
        calls = [{
            "jsonrpc": "2.0", "id": t, "method": "eth_call",
            "params": [{"to": contract, "data": SELECTOR + format(t, "064x")}, "latest"],
        } for t in chunk]
        return rpc_batch(calls, rpc_url)

    owners = {}
    budget = float(os.environ.get("OWNERS_BUDGET", "300"))   # hard cap, seconds
    t0 = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
        for items in ex.map(run, chunks):
            if time.time() - t0 > budget:
                print(f"warn: owners budget ({budget}s) hit; partial result", file=sys.stderr)
                break
            for item in items:
                res = item.get("result"); tid = item.get("id")
                if not res or tid is None or len(res) < 66:
                    continue                              # reverted / nonexistent token
                addr = "0x" + res[-40:]
                if addr != "0x" + "0" * 40:
                    owners[str(tid)] = addr
    print(f"owners: {len(owners)} in {time.time()-t0:.1f}s "
          f"({len(chunks)} batches of {batch}, {workers} workers)", file=sys.stderr)
    return owners
