...
def rpc_batch(calls, rpc_url, tries=4):
...
def fetch_owners():
    ...
    batch    = int(os.environ.get("RPC_BATCH", "400"))
    ...
    for i in range(0, len(ids), batch):
        ...
        for item in rpc_batch(calls, rpc_url):
            res = item.get("result")
            tid = item.get("id")
            ...
            if addr != "0x" + "0" * 40:
                owners[str(tid)] = addr
        time.sleep(float(os.environ.get("RPC_SLEEP", "0.15")))
    return owners
